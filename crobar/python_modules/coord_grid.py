...
tiny = 1.0e-4
# Multiply an array of vectors by a matrix. Order is reversed, and the inv  operator
# transposed, due to how numpy array broadcasting and matrix operations interact.
...
class coord_grid:
	...
	#    origin: The location of the lower left corner of the [0,0,...] pixel in the grid
	...
	def subgrid(self,fac=3):
		return coord_grid(self.dims*fac,self.coords(0.0*self.dims),self.fwd/fac,self.frame)
	# Get the identify grid for this grid -- i.e., it maps indices to themselves. 
	# Indices are coordinates too, ya know! Pairs well with subgrid.
	def identity(self): 
		return coord_grid(self.dims,0.0*self.dims-0.5,np.diag(1+0.0*self.dims),np.arange(len(self.dims)))
	# Returns indices given a set of coordinates. Does not discretize for various reasons. Because
	# coords returns coordinates referenced to the lower left corders of the grid elements, 
	# discretization of these indices should be done with flooring (see flatinds). The domain 
	# of each grid element extends from 0.0 to 0.99999... in grid units.
	def inds(self,coords): return multivec_matmul(self.inv, coords-self.origin) - 0.5
	...
	def coords(self,inds): return multivec_matmul(self.fwd, inds+0.5) + self.origin
	...
	def flatinds(self,vals,coords,thold=0): 
		[inds, keeps] = [list(np.floor(self.inds(coords)+tiny).T.astype(np.int32)), vals>thold]
