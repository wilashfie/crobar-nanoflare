...
def solve(data0, errors0, amat0, guess=None, reg_fac=1, func=None, dfunc=None, ifunc=None, regmat=None, silent=False,
						...
						store_outer_Av=False, conv_chi2 = 1.0e-15, scale_steps=True):
	from scipy.sparse import diags
	from scipy.sparse.linalg import lgmres
	from scipy.linalg import cho_factor, cho_solve
	if(niter is None): niter=40
	...
	if(flatguess and guess is None): guess = np.abs(((1+np.zeros(nsrc))*np.mean(flatdat)/np.mean(amat0*(1+np.zeros(nsrc)))).astype(dtype))
	if(guess is None): guess = guess0
	print(np.min(guess),np.max(guess))
	...
	maxdelta = ifunc(np.max(np.abs(guess0)))-ifunc(0.25*np.max(np.abs(guess0)))
	print('Guess0 min:',np.min(np.abs(guess0)), 'Guess0 max:',np.max(np.abs(guess0)))
	print('Guess0 min:',ifunc(np.min(np.abs(guess0))), 'Guess0 max:',ifunc(np.max(np.abs(guess0))))
	...
	for i in range(0,niter):
		...
		print(np.min(bvec), np.max(bvec), np.min(svec), np.max(svec))
		...
		if(scale_steps):
			print('step scaling:',np.clip(np.max(np.abs(deltas)),None,maxdelta/minstep)/np.max(np.abs(deltas)), np.max(np.abs(deltas)), maxdelta)
		...
		best_step = np.nanargmin((step_loss)[1:nsteps])+1 # First step is zero for comparison purposes...
